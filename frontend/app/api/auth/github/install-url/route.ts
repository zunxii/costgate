...
const STATE_COOKIE = "github_install_state"; // must match setup/callback/route.ts
...
export async function GET(request: Request) {
  ...
  // GitHub sends the user back to the App's configured Setup URL;
  // the setup_url below is returned to the client as a help link only.
  const setup = new URL("/api/auth/github/setup/callback", siteUrl).toString();
